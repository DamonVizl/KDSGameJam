...
public class Fishing : MonoBehaviour
{
    ...
    void OnEnable()
    {
        InputManager.Instance.OnCastPressed += StartCast;
        InputManager.Instance.OnCastReleased += EndCast;
    }
    void OnDisable()
    {
        InputManager.Instance.OnCastPressed -= StartCast;
    ...
    {
        if(PlayerStateMachine.Instance.GetCurrentState() != PlayerState.Moving) return; //shouldn't be required because of the action maps but added regardless.
        CastDistance = 10f;
        _cts?.Cancel();
    ...
    void EndCast()
    {
        ...
        _cts?.Cancel();
        PlayerStateMachine.Instance.TransitionToState(PlayerState.Fishing);
    }
}
